...
import com.qlangtech.tis.extension.INotebookable.InterpreterGroup;
...
public class ZeppelinClient {
    ...
    public String createNote(String notePath, String defaultInterpreterGroup) throws Exception {
        return createNote(notePath, defaultInterpreterGroup, (json) -> {
        });
    }

    /**
     * https://zeppelin.apache.org/docs/0.10.1/usage/rest_api/notebook.html#create-a-new-note
     */
    public String createNoteWithParagraph(String notePath, String defaultInterpreterGroup) throws Exception {
        return createNote(notePath, defaultInterpreterGroup, (json) -> {
            json.put("addingEmptyParagraph", true);
//            kong.unirest.json.JSONArray paragraphs = new kong.unirest.json.JSONArray();
//            kong.unirest.json.JSONObject p = new kong.unirest.json.JSONObject();
//            p.put("title", "default");
//            p.put("text","");
//            paragraphs.put(p);
//            json.put("paragraphs", paragraphs);
     ...
     */
    public String createNote(String notePath, String defaultInterpreterGroup, Consumer<JSONObject> bodyObjectCall) throws Exception {
        JSONObject bodyObject = new JSONObject();
        bodyObject.put("name", notePath);
        bodyObject.put("defaultInterpreterGroup", defaultInterpreterGroup);
        bodyObjectCall.accept(bodyObject);
        HttpResponse<JsonNode> response = Unirest
                .post("/notebook")
    ...
    public static final String TIS_JDBC_GROUP = "jdbc-tis";

    public static String getInterpreterName(String id) {
        return "tis-" + id;
    }

    /**
     * https://my.oschina.net/u/2306127/blog/894606
     */
    public String createJDBCInterpreter(String name) throws Exception {


        return createInterpreter(name, InterpreterGroup.JDBC_GROUP, (props) -> {
            props.add("default.url", "tis_url");
            props.add("default.tisDbName", name);
            props.add("default.driver", name + "driver");
        });


//        final String group = TIS_JDBC_GROUP;
//        JSONObject bodyObject = new JSONObject();
//        bodyObject.put("name", getInterpreterName(name));
//        bodyObject.put("group", group);
//        JSONObject props = new JSONObject();
//
////        val.put("name", "default.url");
////        val.put("value", "jdbc:mysql://192.168.28.200:3306/order2?useUnicode=yes&useCursorFetch=true&useSSL=false&serverTimezone=Asia%2FShanghai&useCompression=true&characterEncoding=utf8&useSSL=false");
//
//        //  addProp(props, "default.url", "jdbc:mysql://192.168.28.200:3306/order2?useUnicode=yes&useCursorFetch=true&useSSL=false&serverTimezone=Asia%2FShanghai&useCompression=true&characterEncoding=utf8&useSSL=false");
//
//        addProp(props, "default.url", "tis_url");
//        addProp(props, "default.tisDbName", name);
//        addProp(props, "default.driver", name + "driver");
//
//        bodyObject.put("properties", props);
//
//        JSONObject igroup = new JSONObject();
//        igroup.put("class", "org.apache.zeppelin.jdbc.TISJDBCInterpreter");
//        igroup.put("name", group);
//        bodyObject.put("interpreterGroup", igroup);
//        bodyObject.put("dependencies", new JSONArray());
//
//        HttpResponse<JsonNode> response = Unirest
//                .post("/interpreter/setting")
//                //.routeParam("interpreter", interpreter)
//                .body(bodyObject.toString())
//                .asJson();
//        checkResponse(response);
...
//        checkJsonNodeStatus(jsonNode);
//
//        JSONObject body = jsonNode.getObject().getJSONObject("body");
...
//        return body.getString("id");
    ...
    /**
     * https://zeppelin.apache.org/docs/0.10.0/usage/rest_api/interpreter.html#delete-an-interpreter-setting
     */
    public void deleteInterpreter(String interpreter) throws Exception {
        if (StringUtils.isEmpty(interpreter)) {
            throw new IllegalArgumentException("param interpreter can not be null");
        }
        HttpResponse<JsonNode> response = Unirest
                .delete("/interpreter/setting/{interpreter}")
                .routeParam("interpreter", getInterpreterName(interpreter))
                //.body(bodyObject.toString())
                .asJson();
        checkResponse(response);
        JsonNode jsonNode = response.getBody();
    ...
    public String createInterpreter(String name, InterpreterGroup group, Consumer<InterpreterProps> propsSetter) throws Exception {

        //  final String group = TIS_JDBC_GROUP;
        JSONObject bodyObject = new JSONObject();
        bodyObject.put("name", getInterpreterName(name));
        bodyObject.put("group", group.getGroupName());
        bodyObject.put("option",) ;
        JSONObject props = new JSONObject();
...
//        val.put("value", "jdbc:mysql://192.168.28.200:3306/order2?useUnicode=yes&useCursorFetch=true&useSSL=false&serverTimezone=Asia%2FShanghai&useCompression=true&characterEncoding=utf8&useSSL=false");
        ...
        propsSetter.accept(new InterpreterProps(props));
...
//        addProp(props, "default.driver", name + "driver");
        ...
        JSONObject igroup = new JSONObject();
        igroup.put("class", group.getInterpreterGroupClass());
        // 作为Id使用
        igroup.put("name", getInterpreterName(name));
        bodyObject.put("interpreterGroup", igroup);
        bodyObject.put("dependencies", new JSONArray());
    ...
    public static class InterpreterProps {
        private final JSONObject props;

        public InterpreterProps(JSONObject props) {
            this.props = props;
        }

        public void add(String key, String val) {
            addProp(props, key, val);
        }

        private void addProp(JSONObject props, String key, String value) {
